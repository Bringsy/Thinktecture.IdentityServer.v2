...
namespace Thinktecture.IdentityServer.Web.Areas.Account
{
    public class CustomNotificationService : INotificationService
    {
        ...
        protected virtual string DoTokenReplacement(string msg, UserAccount user)
        {
            ...
            msg = msg.Replace("{confirmAccountCreateUrl}", appInfo.VerifyAccountUrl + user.VerificationKey + HttpContext.Current.Request.Url.Query);
            msg = msg.Replace("{cancelNewAccountUrl}", appInfo.CancelNewAccountUrl + user.VerificationKey + HttpContext.Current.Request.Url.Query);

            msg = msg.Replace("{confirmPasswordResetUrl}", appInfo.ConfirmPasswordResetUrl + user.VerificationKey + HttpContext.Current.Request.Url.Query);
            msg = msg.Replace("{confirmChangeEmailUrl}", appInfo.ConfirmChangeEmailUrl + user.VerificationKey + HttpContext.Current.Request.Url.Query);
